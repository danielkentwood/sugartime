...
def shape_tandem(tandem_dict):
    ...
    * bolus: size of insulin bolus
    ...
    iob = pd.DataFrame(
        tandem_dict[3], columns=["Type", "EventID", "EventDateTime", "iob"]
    )
    ...
    egv = egv.drop(["DeviceType", "SerialNumber", "Description", "VOID"], axis=1)
...
def shape_clarity(clarity):
    ...
    * clarity: a clean dataframe with carb value measurements
    ...
    clarity = clarity[["Carb Value (grams)", "Timestamp (YYYY-MM-DDThh:mm:ss)"]]
...
def combine_tandem_clarity(clarity, egv, br, iob, bolus):
    ...
    * data: combined dataframe
    ...
    d = pd.concat([egv, br, iob, bolus, clarity[["dt", "carb_grams"]]]).sort_values(
        "dt"
    )
    # reset index
    ...
    data["bolus_units"] = pd.DataFrame(
        d["bolus_units"].astype(float).resample("5T").sum()
    )
    # fill blood glucose and insulin-on-board with linearly interpolated values
    data[["bg", "iob"]] = data[["bg", "iob"]].interpolate(
