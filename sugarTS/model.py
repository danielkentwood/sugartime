...
class Patient:
    ...
    def find_optimal_bolus(self, carbs, carb_t):
        ...
        * Optimal: dict containing:
        ...
        bolus_amounts = list(range(0, carb_targ + 20))
        bolus_times = pd.date_range(
            start=self.Xtest.index[-1] + timedelta(minutes=5),
            end=carb_t, freq="5T"
        )
    ...
    def fit_model(self):
        ...
        Xtrain, ytrain, Xtest, ytest = core.split_data(self.data)
        self.Xtrain = Xtrain
        self.ytrain = ytrain
        self.Xtest = Xtest
        self.ytest = ytest
        ...
        narx_mdl = NARX(
            LinearRegression(),
            auto_order=30, exog_order=[60, 60], exog_delay=[0, 0]
        )
    ...
    def build_x_future(self, carbs, carb_t, bolus, bolus_t):
        ...
        * X_future: (dataframe) matrix containing future carbs and bolus.
        ...
        X_future = pd.DataFrame(
            np.zeros([len(ind), 2]), columns={"carb_grams", "bolus_units"}
        )
        X_future.index = ind
    ...
    def forecast(self, X_future):
        ...
        * yforecast_narx: (array-like) vector containing forecasted
        ...
        yforecast_narx = self.model.forecast(
            self.Xtest, self.ytest, step=steps, X_future=X_future
        )
