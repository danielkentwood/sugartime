...
class Patient:
    ...
    def load_data(self, clarity_filename, tandem_filename):
        """
    ...
    * horizon (int): how far into the future the model will attempt to
    ...
    def dynamic_forecast(self, X, y, start_time, inserts):
        ...
        * dataframe with the forecasted values
        ...
        max_time = max(insert_times) + timedelta(minutes=5 * self.horizon)
        cur_time = start_time - timedelta(minutes=5)
    ...
    def hybrid_forecast(self, X, y, start_time, inserts):
        ...
        * dataframe with the forecasted values
        ...
        insert_times = insert_times + list(
            set([cur_time,
                 max(insert_times) + timedelta(minutes=5 * self.horizon)
                 ]
                )
        )
        insert_times.sort()
        ...
        time_diffs = np.diff(insert_times) / timedelta(minutes=5)
        while np.any(time_diffs > self.horizon):
            for i in np.where(time_diffs > self.horizon)[0]:
                insert_times.append(insert_times[i] + timedelta(minutes=5))
            insert_times.sort()
            time_diffs = np.diff(insert_times) / timedelta(minutes=5)
        ...
        for i in range(len(insert_times) - 1):
            ind = pd.date_range(
                start=insert_times[i] + timedelta(minutes=5),
    ...
    def add_inserts_to_x(self, exog_df, start_time, inserts={}):
        ...
        * a dataframe containing a subset of the exogenous data with
        ...
        ind = pd.date_range(
            start=start_time,
            end=max(insert_times) + timedelta(minutes=5 * self.horizon),
            freq="5T",
        )
        ...
        df = df.loc[(cur_time - timedelta(minutes=5 * (int(back_size)))):]
        return df
    ...
    def find_optimal_bolus(self, carbs, carb_t):
        ...
        * Optimal: dict containing:
        ...
        start_time = self.patient.Xtest.index[-1] + timedelta(minutes=5)
