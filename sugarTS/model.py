...
class Patient:
    ...
    def find_optimal_bolus(self, carbs, carb_t):
        ...
        * Optimal: dict containing:
        ...
        bolus_times = pd.date_range(
            start=self.Xtest.index[-1] + timedelta(minutes=5), end=carb_t, freq="5T"
        )
        # create grid for brute force grid search
        grid = [(r[0], r[1]) for r in itertools.product(bolus_amounts, bolus_times)]
    ...
    def fit_model(self):
        ...
        narx_mdl = NARX(
            LinearRegression(), auto_order=30, exog_order=[60, 60], exog_delay=[0, 0]
        )
        # fit model
